...
int main ( int argc, char* argv[] ) {
    ...
    for (unsigned iter=0; iter<graphNodeCount-1; iter++) {
        ...
        for (graphNode_t source=0; source<graphNodeCount; source++) 
        {
            ...
            {
                for (AdjacencyListNode* adj = adjacencyList[source].next; adj != NULL; adj = adj->next) 
                {
                    if (parents[adj->graphNode] == -1 && adj->weight < minWeight) 
                    {
                        minWeight = adj->weight;
                        minSource = source;
                        minDest = adj->graphNode;
                    }
                }
            }
        }
