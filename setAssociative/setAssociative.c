...
void accessData (
    ...
    cache_set_t currLine = cache[index];

    // Cache hit
    for(size_t i = 0; i < E; i++) 
    {
        if((currLine+i)->valid && (currLine+i)->tag == tag) 
        {
            (*hit_count)++;
            (currLine+i)->recent_use = program_counter;
            return;
        }
    }
    ...
    for(size_t i = 0; i < E; i++) 
    {
        if((currLine+i)->recent_use < leastRecent) 
        {
            leastRecentIndex = i;
            leastRecent = (currLine+i)->recent_use;
        }
    }

    // If cache set line already in use, then record an eviction
    if((currLine+leastRecentIndex)->valid)
        (*eviction_count)++;

    (currLine+leastRecentIndex)->tag = tag;
    (currLine+leastRecentIndex)->valid = true;
